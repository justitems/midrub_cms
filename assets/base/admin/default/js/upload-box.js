...
jQuery(document).ready( function ($) {
    ...
    ********************************/
    ...
    Main.reload_drag_and_drop_areas = function () {
        ...
        $( 'body .theme-drag-and-drop-files' ).each(function() {
            ...
            // Prepare content
            let content = '<div class="theme-upload-main">'
                + '<div class="theme-upload-icon">'
                    + '<span class="iconify" data-icon="fluent:folder-arrow-right-20-regular"></span>'
                + '</div>'
                + '<div class="theme-upload-text">'
                    + '<h2>'
                    ...
                    + '</p>'
                + '</div>'
                + '<div class="theme-upload-btn">'
                    + '<a href="#" class="theme-choose-files">'
                        + Main.translation.theme_choose_file
                + ' </a>'
                + '</div>'                                     
            + '</div>'
            + '<div class="theme-upload-over">'
                + '<div class="theme-upload-icon">'
                    + '<span class="iconify" data-icon="fluent:folder-arrow-right-20-regular"></span>'
                + '</div>'  
            + '</div>'
            + '<div class="theme-upload-drop">'
                + '<div class="theme-upload-progress">'
                    + '<p>'
                        + '0%'
                    + '</p>'
                    + '<div class="progress">'
                ...
                + '</div>'                            
            + '</div>'
            + '<form action="' + $(this).attr('data-upload-form-url') + '" method="post" enctype="multipart/form-data" class="theme-upload-files-form d-none">'
                + '<input type="file" name="file[]" class="theme-upload-files-input" accept="' + extensions + '"' + multiple + '>'
                + for_form
            + '</form>'; 
    ...
    ********************************/
    ...
    $(function () {
        ...
        if ( $('body .theme-drag-and-drop-files').length > 0 ) {
     ...
     * @since   0.0.8.5
    */
    $( document ).on( 'drag dragstart dragend dragover dragenter dragleave drop', 'body .theme-drag-and-drop-files' , function (e) {
        e.preventDefault();
        e.stopPropagation();
        
        // Set drag active class
        $( this ).addClass( 'theme-drag-and-drop-files-over-active' );

        if ( e.handleObj.origType === 'dragleave' || e.handleObj.origType === 'drop' ) {
            
            // Remove drag active class
            $( this ).removeClass( 'theme-drag-and-drop-files-over-active' );
            ...
            if ( e.handleObj.origType === 'drop' ) {
                ...
                if (typeof e.originalEvent.dataTransfer.files[0] !== 'undefined') {
                    ...
                    $( this ).addClass( 'theme-drag-and-drop-files-drop-active' );
    ...
    * @since   0.0.8.5
    */    
    $( document ).on( 'change', 'body .theme-drag-and-drop-files [type="file"]', function (e) {
        ...
        // Submit form
        $(this).closest('.theme-drag-and-drop-files').find('form').submit();
     ...
     * @since   0.0.8.5
    */
    $( document ).on( 'click', 'body .theme-drag-and-drop-files .theme-choose-files', function (e) {
        e.preventDefault();
    
        // Browse
        $(this).closest('.theme-drag-and-drop-files').find('[type="file"]').click();
    ...
    ********************************/
     ...
     * @since   0.0.8.5
     */
    $(document).on('submit', 'body .theme-upload-files-form', function (e) {
        e.preventDefault();
        ...
        if ( typeof Main.uploaded_files !== 'undefined' ) {
            ...
            // Get files
            var files = $this.find('.theme-upload-files-input')[0].files;
        ...
        if ( typeof files[0] !== 'undefined' ) {
            ...
            Main.save_files(files, function(files) {
                ...
                // Set the action
                form.append('action', $this.closest('.theme-drag-and-drop-files').attr('data-upload-form-action'));

                // Upload files
                $.ajax({

                    // Set url
                    url: $this.closest('.theme-drag-and-drop-files').attr('data-upload-form-url'),
                    ...
                    xhr: function () {
                        ...
                        xhr.upload.onprogress = function (e) {
                            ...
                            if (e.lengthComputable) {
                                ...
                                // Set percentage
                                $this.closest('.theme-drag-and-drop-files').find('.theme-upload-progress p').text(percentage + '%');

                                // Set progress
                                $this.closest('.theme-drag-and-drop-files').find('.theme-upload-progress .progress-bar').attr('aria-valuenow', percentage).attr('style', 'width: ' + percentage + '%');
                    ...
                    success: function (data) {
                        ...
                        if ( data.success === true ) {
                            ...
                            Main.call_object('success', data, $this.closest('.theme-drag-and-drop-files').attr('data-upload-form-action'));
                        
                        } else {
                        
                            // Call the response function and return error message
                            Main.call_object('error', data, $this.closest('.theme-drag-and-drop-files').attr('data-upload-form-action'));
                    ...
                    complete: function( jqXHR, textStatus, errorThrown ) {
                        ...
                        $this.closest('.theme-drag-and-drop-files').removeClass('theme-drag-and-drop-files-drop-active');
