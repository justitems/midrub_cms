...
class User {
    ...
    public function view() {
        ...
        set_the_title($this->CI->lang->line('team'));
        ...
        if ( $this->CI->input->get('p', true) ) {
            ...
            // Get user team's pages
            $user_team_pages = the_user_team_pages();

            // Verify if team's pages exists
            if ($user_team_pages) {

                // List all user's pages
                foreach ($user_team_pages as $user_team_page) {

                    // Get page slug
                    $page_slug = array_keys($user_team_page);
                    ...
                    if ( $page_slug[0] === $this->CI->input->get('p', true) ) {
                        ...
                        if (isset($user_team_page[$page_slug[0]]['css_urls'])) {

                            // Verify if the css_urls array is not empty
                            if ($user_team_page[$page_slug[0]]['css_urls']) {

                                // List all css links
                                foreach ($user_team_page[$page_slug[0]]['css_urls'] as $css_link_array) {
                        ...
                        if (isset($user_team_page[$page_slug[0]]['js_urls'])) {

                            // Verify if the js_urls array is not empty
                            if ($user_team_page[$page_slug[0]]['js_urls']) {

                                // List all js links
                                foreach ($user_team_page[$page_slug[0]]['js_urls'] as $js_link_array) {
