...
jQuery(document).ready( function ($) {
    ...
    ********************************/
    ...
    Main.load_team_roles = function (page) {
        ...
        var data = {
            ...
            key: $('.member .search-roles').val(),
            page: page
        };

        // Set CSRF
        data[$('.member .update-member').attr('data-csrf')] = $('input[name="' + $('.member .update-member').attr('data-csrf') + '"]').val();
    ...
    ********************************/
     ...
     * @since   0.0.8.2
     */
    $(document).on('keyup', '.member .search-roles', function (e) {
        e.preventDefault();
     ...
     * @since   0.0.8.2
     */
    $(document).on('click', '.member .selected-member-role', function (e) {
        e.preventDefault();
     ...
     * @since   0.0.8.2
     */
    $(document).on('click', '.member .new-role', function (e) {
        e.preventDefault();

        // Show or hide
        $('.member .member-role-create').slideToggle('slow');
     ...
     * @since   0.0.8.2
     */
    $(document).on('click', '.member .btn-create-role', function (e) {
        e.preventDefault();

        // Create an object with form data
        var data = {
            action: 'team_create_role',
            role: $('.update-member .role-name').val()
        };
     ...
     * @since   0.0.8.2
     */
    $(document).on('click', '.member .dropdown-menu div > a', function (e) {
        e.preventDefault();
    ...
    ********************************/
    ...
    Main.methods.team_new_member_response = function ( status, data ) {
        ...
        if ( status === 'success' ) {
            ...
            $('.member .selected-member-role').html(data.words.select_a_role + ' ' + dropdown_icon);
            
            // Display the select members status words
            $('.member .selected-member-status').html(data.words.select_a_status + ' ' + dropdown_icon);

            // Uncheck email notification
            $('.member #team-member-send-email').prop( 'checked', false );
    ...
    Main.methods.team_all_roles = function ( status, data ) {
        ...
        if ( status === 'success' ) {
            ...
            // Display the roles
            $('.member .member-roles > div').html(roles);
            ...
            // Display the message
            $('.member .member-roles > div').html(message);
    ...
    ********************************/
     ...
     * @since   0.0.8.2
     */
    $('.member .update-member').submit(function (e) {
        e.preventDefault();
        ...
        var data = {
            ...
            about: $(this).find('.about-member').val()
        };
