...
namespace MidrubBase\User\Components\Collection\Plans\Helpers; 
    ...
    public function verify_coupon() {
        ...
        if ( $this->CI->input->post() ) {
            ...
            if ($this->CI->form_validation->run() === false) {
                ...
                $data = array(
                    'success' => FALSE,
                    'message' => $this->CI->lang->line('plans_coupon_code_missing')
                );
                ...
                if ( $coupon_code ) {
                    ...
                    if ( ($coupon_code[0]['type'] > 0) && ($coupon_code[0]['count'] > 0) ) {
                        ...
                        $response = array(
                            'success' => FALSE,
                            'message' => $this->CI->lang->line('plans_coupon_code_is_not_valid')
                        );
                        ...
                        $response = array(
                            'success' => TRUE,
                            'message' => $this->CI->lang->line('plans_coupon_code_is_valid'),
                            'value' => $coupon_code[0]['value'],
                            'words' => array(
                                'discount' => $this->CI->lang->line('plans_discount')
                            )
                        );
