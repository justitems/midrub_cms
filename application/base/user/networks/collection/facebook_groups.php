...
/**
 * Facebook_groups class - allows users to connect to their Facebook Groups
 *
 * @category Social
 * @package  Midrub
 * @author   Scrisoft <asksyn@gmail.com>
 ...
 * @link     https://www.midrub.com/
 */
class Facebook_groups implements CmsBaseUserInterfaces\Networks {
    ...
    public function __construct() {
        ...
        $this->app_id = md_the_option('network_facebook_groups_app_id');
        
        // Set the Facebook App Secret
        $this->app_secret = md_the_option('network_facebook_groups_app_secret');

        // Set the Facebook Api Version
        $this->api_version = md_the_option('network_facebook_groups_api_version')?md_the_option('network_facebook_groups_api_version'):$this->api_version;
    ...
    public function connect() {
        ...
        if ( md_the_option('network_facebook_groups_permissions') ) {

            // Get the permissions
            $the_permissions = md_the_option('network_facebook_groups_permissions');
        ...
        $params = array(
            ...
            'state' => time(),
            'response_type' => 'code',
            'redirect_uri' => site_url('user/callback/facebook_groups'),
    ...
    public function callback($token = null) {
        ...
        if ($this->CI->input->post()) {
            ...
            if ($this->CI->form_validation->run() == false) {
                ...
                // Get user data
                $response = json_decode(file_get_contents('https://graph.facebook.com/' . $this->api_version . '/me/groups?limit=200&fields=administrator,name&access_token=' . $token), true);

                // Get connected facebook groups
                $the_connected_facebook_groups = $this->CI->base_model->the_data_where(
                    'networks',
                    'network_id, net_id',
                    array(
                ...
                // Verify if user has connected facebook groups
                if ( $the_connected_facebook_groups ) {

                    // List all connected facebook groups
                    foreach ( $the_connected_facebook_groups as $connected ) {
                ...
                if ( $net_ids ) {
                    ...
                    if ( isset($response['data'][0]['id']) ) {
                        ...
                        // Connected networks
                        $connected_networks = $the_connected_facebook_groups?array_column($the_connected_facebook_groups, 'network_id', 'net_id'):array();
            ...
            $the_facebook_groups = json_decode(file_get_contents('https://graph.facebook.com/' . $this->api_version . '/me/groups?limit=200&fields=administrator,name&access_token=' . $the_token['access_token']), true);

            // Verify if user has groups
            if ( isset($the_facebook_groups['data'][0]['id']) ) {
                ...
                // Save page
                for ( $y = 0; $y < count($the_facebook_groups['data']); $y++ ) {

                    // Set item
                    $items[$the_facebook_groups['data'][$y]['id']] = array(
                        'net_id' => $the_facebook_groups['data'][$y]['id'],
                        'name' => $the_facebook_groups['data'][$y]['name'],
                        'label' => '',
                        'connected' => FALSE
                    );

                    // Verify if this Facebook Page is connected
                    if ( in_array($the_facebook_groups['data'][$y]['id'], $net_ids) ) {

                        // Set as connected
                        $items[$the_facebook_groups['data'][$y]['id']]['connected'] = TRUE;
                ...
                $params = array(
                    ...
                    'network_name' => 'facebook_groups',
                    'items' => $items,
                    'connect' => $this->CI->lang->line('user_network_facebook_groups'),
                ...
                echo $this->CI->load->ext_view(
                    ...
                    'network_error',
                    array(
                        'message' => $this->CI->lang->line('user_network_no_groups_found')
                    ),
                    TRUE
                );
                exit();
    ...
    public function info() {
        ...
        return array(
            'network_name' => 'Facebook Groups',
            'network_version' => '0.1',
            'network_configuration' => array(
                'fields' => array(
                    array(
                        'field_slug' => 'network_facebook_groups_enabled',
                        'field_type' => 'checkbox',
                        'field_words' => array(
                            'field_title' => 'Enable',
                            'field_description' => 'By enabling this network you will see it in the plans groups. You have to enable it there too for the wanted plans.'
                        ),
                        'field_params' => array(
                            'checked' => md_the_option('network_facebook_groups_enabled')?md_the_option('network_facebook_groups_enabled'):0
                        )
    
                    ),
                    array(
                        'field_slug' => 'network_facebook_groups_app_id',
                        'field_type' => 'text',
                        'field_words' => array(
                        ...
                        'field_params' => array(
                            ...
                            'value' => md_the_option('network_facebook_groups_app_id')?md_the_option('network_facebook_groups_app_id'):'',
                            'disabled' => false
                        )

                    ),
                    array(
                        'field_slug' => 'network_facebook_groups_app_secret',
                        'field_type' => 'text',
                        'field_words' => array(
                        ...
                        'field_params' => array(
                            ...
                            'value' => md_the_option('network_facebook_groups_app_secret')?md_the_option('network_facebook_groups_app_secret'):'',
                            'disabled' => false
                        )

                    ),
                    array(
                        'field_slug' => 'network_facebook_groups_api_version',
                        'field_type' => 'text',
                        'field_words' => array(
                        ...
                        'field_params' => array(
                            ...
                            'value' => md_the_option('network_facebook_groups_api_version')?md_the_option('network_facebook_groups_api_version'):'',
                            'disabled' => false
                        )

                    ),
                    array(
                        'field_slug' => 'network_facebook_groups_permissions',
                        'field_type' => 'text',
                        'field_words' => array(
                        ...
                        'field_params' => array(
                            ...
                            'value' => md_the_option('network_facebook_groups_permissions')?md_the_option('network_facebook_groups_permissions'):'',
                            'disabled' => false
                        )

                    ),
                    array(
                        'field_slug' => 'network_facebook_groups_redirect_url',
                        'field_type' => 'text',
                        'field_words' => array(
                        ...
                        'field_params' => array(
                            'placeholder' => "",
                            'value' => site_url('user/callback/facebook_groups'),
                            'disabled' => true
                        )
