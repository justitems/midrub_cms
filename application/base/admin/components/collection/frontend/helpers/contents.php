...
class Contents {
    ...
    public function save_contents() {
        ...
        if ( $this->CI->input->post() ) {
            ...
            foreach ( $languages as $language ) {
                ...
                // Get language value
                $langs[$only_dir] = $this->CI->input->post($only_dir, FALSE);
    ...
    public function frontend_get_contents_by_category() {
        ...
        if ( $this->CI->input->post() ) {
            ...
            if ($this->CI->form_validation->run() !== false ) {
                ...
                $contents = $this->CI->base_contents->the_contents( $args );
                ...
                if ( $contents ) {
                    ...
                    $total = $this->CI->base_contents->the_contents($args);
                    ...
                    $data = array(
                        ...
                        'page' => ($page + 1),
                        'words' => array(
                            'results' => $this->CI->lang->line('frontend_results'),
                            'of' => $this->CI->lang->line('frontend_of'),
                            'published' => $this->CI->lang->line('frontend_published'),
    ...
    public function frontend_delete_content() {
        ...
        $content = $this->CI->input->get('content', TRUE);  

        // Verify if content's id exists
        if ( $content ) {

            // Delete content
            $delete = $this->CI->base_contents->delete_content($content);

            // Verify if the content was deleted
            if ( $delete ) {
