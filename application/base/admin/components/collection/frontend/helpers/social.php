                ...
                if ($zip->open(CMS_BASE_ADMIN_COMPONENTS_FRONTEND . 'temp/' . $this->CI->input->get('file_name') . '.zip') === TRUE) {
                    ...
                    if ( file_exists(CMS_BASE_ADMIN_COMPONENTS_FRONTEND . 'temp/installation.json') ) {
                        ...
                        if ( isset($installation['php']) && isset($installation['category']) ) {
                            ...
                            if ( $installation['category'] === 'auth_networks' ) {
                                ...
                                if ( file_exists(CMS_BASE_ADMIN_COMPONENTS_FRONTEND . 'temp/' . $installation['php']) ) {
                                    ...
                                    if ($zip->open(CMS_BASE_ADMIN_COMPONENTS_FRONTEND . 'temp/' . $installation['php']) === TRUE) {
                                        ...
                                        if ( $zip->numFiles > 0 ) {
                                            ...
                                            if ( file_exists(CMS_BASE_PATH . 'admin/components/collection/frontend/networks/collection/' . $network_dir) ) {

                                                // Prepare the error message
                                                $data = array(
                                                    'success' => FALSE,
                                                    'message' => $this->CI->lang->line('frontend_network_already_installed')
                                                );
                                                ...
                                                // Extract the zip
                                                $zip->extractTo(CMS_BASE_PATH . 'admin/components/collection/frontend/networks/collection/');

                                                // Prepare the succes message
                                                $data = array(
                                                    'success' => TRUE,
                                                    'message' => $this->CI->lang->line('frontend_network_was_installed')
                                                );
                                ...
                                $data = array(
                                    'success' => FALSE,
                                    'message' => $this->CI->lang->line('frontend_network_not_supported')
                                );
                            ...
                            $data = array(
                                'success' => FALSE,
                                'message' => $this->CI->lang->line('frontend_network_can_not_be_installed')
                            );
