        ...
        if ( $this->CI->input->post() ) {
            ...
            if ($this->CI->form_validation->run() === false ) {
                ...
                if ( is_dir(APPPATH . 'base/user/components/collection/' . $slug) ) {
                    ...
                    if ( !empty($component['updates_code']) && !$code ) {
                    ...
                    } else if ( !empty($component['updates_code']) && $code ) {
                    ...
                    // Get the updates
                    $get_updates = json_decode(get($component['updates_url'] . $updates_code), true);
            ...
            if ($this->CI->form_validation->run() === false ) {
                ...
                if ( is_dir(APPPATH . 'base/user/components/collection/' . $slug) ) {
                    ...
                    if ( $zip->open($zip_name) === TRUE ) {
                        ...
                        if ( file_exists($this->component_dir . '/temp/updates.json') ) {

                            // Get files to updates
                            $updates = json_decode(file_get_contents($this->component_dir . '/temp/updates.json'), true);
                ...
                if ( is_dir(APPPATH . 'base/user/components/collection/' . $slug) ) {
                    ...
                    if ( file_exists($this->component_dir . '/temp/updates.json') ) {
                        ...
                        if ( isset($updates['files']) ) {
                            ...
                            if ( file_exists($this->component_dir . '/updates.json') ) {

                                // Delete the existing updates.json
                                unlink($this->component_dir . '/updates.json');

                            }

                            // Copy the updates.json in the main directory
                            copy($this->component_dir . '/temp/updates.json', $this->component_dir . '/updates.json');
        ...
        if ( $this->copied ) {
            ...
            if ( isset($get_last['files']) ) {
                ...
                if ($error_check == 0) {
                    ...
                    // Verify if old updates.json exists
                    if ( file_exists($component_dir . '/updates.json') ) {

                        // Delete the existing updates.json
                        unlink($component_dir . '/updates.json');
